...
@Component("personConverter") public class PersonConverter implements Function<Person, PersonDto> {
  ...
  @Override public PersonDto apply(@NotNull final Person person) {
    ...
    return new PersonDto.Builder().setId(person.getId()).setRole(new CustomPersonRoleConverter().apply(person.getRole())).setPersonalData(
        new CustomPersonalDataConverter().apply(person.getPersonalData())).setContacts(new CustomContactConverter().apply(
        person.getContacts())).setSubjects(person.getSubjects().stream().map(customSubjectConverter).collect(
        Collectors.toSet())).setGroup(new CustomGroupConverter().apply(person.getGroup())).build();
  }
  ...
  private class CustomPersonRoleConverter implements Function<PersonRole, PersonRoleDto> {
    ...
    @Override public PersonRoleDto apply(PersonRole personRole) {
  ...
  private class CustomPersonalDataConverter implements Function<PersonalData, PersonalDataDto> {
    ...
    @Override public PersonalDataDto apply(PersonalData personalData) {
      return new PersonalDataDto(personalData.getId(),
  ...
  private class CustomSubjectConverter implements Function<Subject, SubjectDto> {
    ...
    @Override public SubjectDto apply(Subject subject) {
