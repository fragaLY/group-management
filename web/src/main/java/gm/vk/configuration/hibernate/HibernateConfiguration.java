...
@Configuration @EnableTransactionManagement @EnableJpaRepositories({
    ...
    "gm.vk.core.dao.user" }) @PropertySource("classpath:properties/jdbc.properties") @ComponentScan({
    "gm.vk.core", "gm.vk.service" }) public class HibernateConfiguration {
  ...
  @Autowired public HibernateConfiguration(Environment environment) {
    this.environment = environment;
  }

  @Bean(name = "dataSource") public DataSource getDataSource() {

    return new EmbeddedDatabaseBuilder().continueOnError(true).setType(EmbeddedDatabaseType.H2).addScript(
        CREATE_SCHEMAS).addScripts(CREATE_ADDRESS_TABLES, CREATE_CONTACTS_TABLES).addScripts(
        CREATE_PERSONAL_DATA_TABLES,
        CREATE_PERSON_ROLE_TABLES,
        CREATE_PERSON_TABLES,
        CREATE_USER_TABLES).addScripts(CREATE_GRADE_TABLES,
        CREATE_EXAMINATION_TYPE_TABLES,
        CREATE_EXAMINATION_TABLES,
        CREATE_SUBJECT_TABLES).addScripts(CREATE_COURSE_TABLES,
        CREATE_SEMESTER_TABLES,
        CREATE_FACULTY_TABLES,
        CREATE_STUDENTGROUP_TABLES).addScript(CREATE_PERSONID_SUBJECTID_TABLES).addScript(
        CREATE_PERSONID_GROUPID_TABLES).addScript(CREATE_SUBJECTID_GROUPID_TABLES).addScripts(INIT_ADDRESS_TABLE,
        INIT_CONTACTS_TABLE).addScripts(INIT_PERSONAL_DATA_TABLE,
        INIT_PERSON_ROLE_TABLE,
        INIT_PERSON_TABLE,
        INIT_USER_TABLE).addScripts(INIT_GRADE_TABLE,
        INIT_EXAMINATION_TYPE_TABLE,
        INIT_EXAMINATION_TABLE,
        INIT_SUBJECT_TABLE).addScripts(INIT_COURSE_TABLE,
        INIT_SEMESTER_TABLE,
        INIT_FACULTY_TABLE,
        INIT_STUDENTGROUP_TABLE).addScript(INIT_PERSONID_SUBJECTID_TABLE).addScript(
        INIT_PERSONID_GROUPID_TABLE).addScript(INIT_SUBJECTID_GROUPID_TABLE).build();
  }
  ...
  private Properties getHibernateProperties() {
    ...
    properties.put(HIBERNATE_PROPERTY_FORMAT_SQL,
        environment.getRequiredProperty(HIBERNATE_PROPERTY_FORMAT_SQL));
    properties.put(HIBERNATE_PROPERTY_GENERATE_STATISTICS,
        environment.getRequiredProperty(HIBERNATE_PROPERTY_GENERATE_STATISTICS));
    properties.put(HIBERNATE_PROPERTY_MAX_POOL_SIZE,
