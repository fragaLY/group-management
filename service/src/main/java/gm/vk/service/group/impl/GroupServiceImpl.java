...
@Service("groupService") public class GroupServiceImpl implements GroupService {

    private static final Logger LOG = LoggerFactory.getLogger(GroupServiceImpl.class);

    @Autowired private GroupDao groupDao;

    @Autowired private GroupConverter groupConverter;

    @Autowired private GroupDtoConverter groupDtoConverter;

    @Override @Transactional(readOnly = true, propagation = Propagation.REQUIRED, rollbackFor = Throwable.class) public List<GroupDto> findAll() {

        LOG.info("Gets all Groups");

        return groupDao.findAll().stream().filter(Objects::nonNull).map(groupConverter).collect(Collectors.toList());
    }

    @Override @Transactional(readOnly = true, propagation = Propagation.REQUIRED, rollbackFor = Throwable.class) public GroupDto findOne(
    ...
    @Override @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Throwable.class) public GroupDto save(
        @NotNull final GroupDto group) {
